...
def plot_comparison(raw_data, ackf_results, ukf_results, real_angels):
    ...
    real_wellbore = np.array(
        [
            [r["real_azimuth"], r["real_inclination"], r["real_toolface"]]
            for r in ackf_results
        ]
    )
    ...
    for i in range(len(raw_data)):
        ...
        azimuth_obs, inclination_obs, toolface_obs, _ = attitude_to_wellbore_quaternion(
            raw_angles[i, 0], raw_angles[i, 1], raw_angles[i, 2]
        )
    ...
    for i in range(3):
        ...
        ax1 = axes2[i, 0]
        ax1.plot(
            x_axis, raw_wellbore[:, i], "b-", alpha=0.7, linewidth=0.8, label="观测数据"
        )
        ...
        ax2.plot(
            x_axis, raw_wellbore_error, "b-", alpha=0.7, linewidth=0.8, label="观测误差"
        )
...
def print_metrics_comparison(raw_data, ackf_results, ukf_results, real_angels):
    ...
    for i, angle in enumerate(angle_names):
        ...
        print(
            f"  ACKF改善:      {((ackf_metrics['rmse'][0][i] - ackf_metrics['rmse'][1][i])/ackf_metrics['rmse'][0][i]*100):.2f}%"
        )
        print(
            f"  UKF改善:       {((ukf_metrics['rmse'][0][i] - ukf_metrics['rmse'][1][i])/ukf_metrics['rmse'][0][i]*100):.2f}%"
        )
    ...
    for i, angle in enumerate(wellbore_names):
        ...
        ackf_improvement = (
            (
                (raw_wellbore_rmse[i] - ackf_wellbore_rmse[i])
                / raw_wellbore_rmse[i]
                * 100
            )
            if raw_wellbore_rmse[i] != 0
            else 0
        )
        ukf_improvement = (
            ((raw_wellbore_rmse[i] - ukf_wellbore_rmse[i]) / raw_wellbore_rmse[i] * 100)
            if raw_wellbore_rmse[i] != 0
            else 0
        )
        ackf_vs_ukf = (
            (
                (ukf_wellbore_rmse[i] - ackf_wellbore_rmse[i])
                / ukf_wellbore_rmse[i]
                * 100
            )
            if ukf_wellbore_rmse[i] != 0
            else 0
        )
...
def export_data_to_excel(
    ...
    ukf_results,
    real_angels,
    filename="no15_filter_comparison_data.xlsx",
):
    """
    导出所有图表数据到Excel文件
    """
    ...
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        ...
        # 计算井眼角度RMSE
        raw_wellbore_rmse = np.sqrt(
            np.mean((raw_wellbore - real_wellbore) ** 2, axis=0)
        )
        ackf_wellbore_rmse = np.sqrt(
            np.mean((ackf_wellbore - real_wellbore) ** 2, axis=0)
        )
        ukf_wellbore_rmse = np.sqrt(
            np.mean((ukf_wellbore - real_wellbore) ** 2, axis=0)
        )
        ...
        metrics_summary = pd.DataFrame(
            {
                ...
                "ACKF改善率(%)": [
                    (
                        (ackf_metrics["rmse"][0][0] - ackf_metrics["rmse"][1][0])
                        / ackf_metrics["rmse"][0][0]
                        * 100
                    ),
                    (
                        (ackf_metrics["rmse"][0][1] - ackf_metrics["rmse"][1][1])
                        / ackf_metrics["rmse"][0][1]
                        * 100
                    ),
                    (
                        (ackf_metrics["rmse"][0][2] - ackf_metrics["rmse"][1][2])
                        / ackf_metrics["rmse"][0][2]
                        * 100
                    ),
                    (
                        (raw_wellbore_rmse[0] - ackf_wellbore_rmse[0])
                        / raw_wellbore_rmse[0]
                        * 100
                        if raw_wellbore_rmse[0] != 0
                        else 0
                    ),
                    (
                        (raw_wellbore_rmse[1] - ackf_wellbore_rmse[1])
                        / raw_wellbore_rmse[1]
                        * 100
                        if raw_wellbore_rmse[1] != 0
                        else 0
                    ),
                    (
                        (raw_wellbore_rmse[2] - ackf_wellbore_rmse[2])
                        / raw_wellbore_rmse[2]
                        * 100
                        if raw_wellbore_rmse[2] != 0
                        else 0
                    ),
                ],
                "UKF改善率(%)": [
                    (
                        (ukf_metrics["rmse"][0][0] - ukf_metrics["rmse"][1][0])
                        / ukf_metrics["rmse"][0][0]
                        * 100
                    ),
                    (
                        (ukf_metrics["rmse"][0][1] - ukf_metrics["rmse"][1][1])
                        / ukf_metrics["rmse"][0][1]
                        * 100
                    ),
                    (
                        (ukf_metrics["rmse"][0][2] - ukf_metrics["rmse"][1][2])
                        / ukf_metrics["rmse"][0][2]
                        * 100
                    ),
                    (
                        (raw_wellbore_rmse[0] - ukf_wellbore_rmse[0])
                        / raw_wellbore_rmse[0]
                        * 100
                        if raw_wellbore_rmse[0] != 0
                        else 0
                    ...
                    (
                        (raw_wellbore_rmse[1] - ukf_wellbore_rmse[1])
                        / raw_wellbore_rmse[1]
                        * 100
                        if raw_wellbore_rmse[1] != 0
                        else 0
                    ...
                    (
                        (raw_wellbore_rmse[2] - ukf_wellbore_rmse[2])
                        / raw_wellbore_rmse[2]
                        * 100
                        if raw_wellbore_rmse[2] != 0
                        else 0
...
if __name__ == "__main__":
    ...
    # 生成测试数据
    data_list, real_angels = generate_test_data(n_points=2000, noise_level=1.0)
