import numpy as np
import pandas as pd  # 在文件顶部添加这个导入
...
def plot_sensor_data_comparison(data_list, ackf_results, ukf_results, true_sensor_data):
    ...
    fig.suptitle("传感器数据滤波效果比较", fontsize=16, fontweight="bold")
...
def plot_angle_comparison(raw_data, ackf_results, ukf_results, real_angels):
    ...
    fig.suptitle("角度滤波效果比较", fontsize=16, fontweight="bold")
...
def plot_paper_style_figures(
    ...
    # 创建角度的论文风格图
    fig1, axes1 = plt.subplots(6, 1, figsize=(12, 16))
    fig1.suptitle("改进ACKF角度滤波结果", fontsize=16, fontweight="bold")
    ...
    for i, (data, label, ylabel) in enumerate(subplot_data_angles):
        axes1[i].plot(x_axis, data, color=colors[i], linewidth=1.0, label=label)
        axes1[i].plot(
            x_axis,
            real_angles[:, i // 2],
            "k--",
        ...
        axes1[i].set_xlim(0, len(x_axis))

    axes1[-1].set_xlabel("测量点数据点")
    plt.tight_layout()
    plt.show()

    # 创建传感器数据的论文风格图
    fig2, axes2 = plt.subplots(6, 1, figsize=(12, 16))
    fig2.suptitle("改进ACKF传感器数据滤波结果", fontsize=16, fontweight="bold")
    ...
    for i in range(6):
        ...
        if i < 3:  # 加速度计数据
            ...
            axes2[i].plot(
                x_axis, ackf_sensor[:, i], "red", linewidth=1.2, label="ACKF滤波数据"
            )
        else:  # 磁力计数据
            axes2[i].plot(
                x_axis,
        ...
        axes2[i].plot(
            x_axis, true_sensor[:, i], "k--", linewidth=1.5, alpha=0.7, label="真实数据"
        )
        axes2[i].set_ylabel(sensor_labels[i])
        axes2[i].legend()
...
def export_sensor_data_to_excel(
    ...
    ukf_results,
    true_sensor_data,
    filename="test14_sensor_data_comparison.xlsx",
):
    """
    导出传感器数据比较到Excel
    """
    ...
    print(f"传感器数据已导出到: {filename}")


def export_angle_data_to_excel(
    raw_data,
    ackf_results,
    ukf_results,
    real_angels,
    filename="test14_angle_data_comparison.xlsx",
):
    """
    导出角度数据比较到Excel
    """
    ...
    df = pd.DataFrame(df_data)
    df.to_excel(filename, index=False)
    print(f"角度数据已导出到: {filename}")


def export_paper_style_data_to_excel(
    raw_data,
    ackf_results,
    real_angels,
    data_list,
    true_sensor_data,
    filename="test14_paper_style_data.xlsx",
):
    """
    导出论文风格数据到Excel（包含角度和传感器数据）
    """
    # 角度数据
    raw_angles = np.array(raw_data)
    ackf_angles = np.array([[r["roll"], r["pitch"], r["yaw"]] for r in ackf_results])
    ...
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        ...
        for i in range(len(raw_data)):
            angle_data.append(
                {
                    "Index": i,
                    "roll_raw": raw_angles[i, 0],
                    "roll_ackf": ackf_angles[i, 0],
                    "roll_true": real_angles[i, 0],
                    "pitch_raw": raw_angles[i, 1],
                    "pitch_ackf": ackf_angles[i, 1],
                    "pitch_true": real_angles[i, 1],
                    "yaw_raw": raw_angles[i, 2],
                    "yaw_ackf": ackf_angles[i, 2],
                    "yaw_true": real_angles[i, 2],
                }
            )
...
if __name__ == "__main__":
    ...
    # 导出数据到Excel
    print("\n导出数据到Excel...")
    export_sensor_data_to_excel(data_list, ackf_results, ukf_results, true_sensor_data)
    export_angle_data_to_excel(raw_data, ackf_results, ukf_results, real_angels)
    export_paper_style_data_to_excel(
        raw_data, ackf_results, real_angels, data_list, true_sensor_data
    )
